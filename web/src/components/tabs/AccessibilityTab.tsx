...
function PageA11yCard({ data, defaultOpen = false }: { data: any; defaultOpen?: boolean }) {
        ...
        <div className="details-content a11y-page-body">
          ...
          <div className="a11y-section">
            ...
            {(data.headings?.headings || []).length > 0 ? (
              <div className="table-responsive">
                <table className="link-table">
                  ...
                  <tbody>
                    {data.headings.headings.map((h: any, i: number) => (
                      <tr key={i}>
            ...
            {(data.headings?.issues || []).length > 0 && (
              <div className="a11y-issues-list mt-4">
                {data.headings.issues.map((issue: any, i: number) => (
          ...
          <div className="a11y-section">
            ...
            {(data.aria?.issues || []).length > 0 ? (
              <div className="a11y-issues-list">
                {data.aria.issues.map((issue: any, i: number) => (
                  <div key={i} className="a11y-issue-row">
                    <SeverityBadge severity={issue.severity} />
                    <div>
          ...
          <div className="a11y-section">
            ...
            {(data.forms?.forms || []).length > 0 ? (
              data.forms.forms.map((form: any, i: number) => (
                <div key={i} className="a11y-form-card glass-panel">
                  ...
                  {(form.issues || []).length > 0 && (
                    <div className="a11y-issues-list mt-4">
                      {form.issues.map((issue: any, j: number) => (
                  ...
                  {form.unlabeled_inputs?.length > 0 && (
                    <div className="table-responsive mt-4">
                      <table className="link-table">
                        <thead><tr><th>Tag</th><th>Type</th><th>Name</th><th>HTML</th></tr></thead>
                        <tbody>
                          {form.unlabeled_inputs.map((inp: any, j: number) => (
                            <tr key={j}>
                              <td>{inp.tag}</td>
                              <td>{inp.type}</td>
                              <td>{inp.name || "-"}</td>
...
export default function AccessibilityTab({ accessibility }: { accessibility: any }) {
  const mainPage = accessibility?.main_page;
  const perUrl: any[] = accessibility?.per_url || [];
