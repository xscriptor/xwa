...
export class VulnerabilitiesFindingsReportComponent implements OnInit, OnDestroy {
  ...
  exportFindingsAsPdf() {
    ...
    doc.setFontSize(14);
    doc.text(`${t('Samurai Vulnerability Report')} | Scan #${scanId}`, 40, 36);
    doc.setFontSize(10);
    doc.text(`${t('Target:')} ${target}`, 40, 54);
    doc.text(`${t('Exported:')} ${new Date().toISOString()}`, 40, 68);
    doc.text(
      `${t('Current Filters:')} severity=${this.severityFilter} type=${this.typeFilter} | ${t('Full dataset exported')}`,
      40,
      82
    );
    ...
    autoTable(doc, {
      startY: 96,
      head: [[t('Node'), t('URL'), t('Type'), t('Severity'), 'CVSS', t('Description'), t('PoC'), t('Matches Filters')]],
      body: rows.map((row) => [
        row['node_type'] || '',
        row['url'] || '',
        row['finding_type'] || '',
