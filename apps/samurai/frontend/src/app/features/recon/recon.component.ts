...
export class ReconComponent implements OnInit, OnDestroy {
  ...
  exportReconAsPdf(): void {
    ...
    doc.setFontSize(14);
    doc.text(t('Samurai Web Recon Report'), 40, 36);
    doc.setFontSize(10);
    doc.text(`${t('Target:')} ${payload.target || 'unknown'}`, 40, 54);
    doc.text(`${t('Modules:')} ${payload.selectedModules.join(', ')}`, 40, 68);
    doc.text(`${t('Exported:')} ${payload.exportedAt}`, 40, 82);

    autoTable(doc, {
      startY: 96,
      head: [[t('Section'), t('Item'), t('Status'), t('Details')]],
